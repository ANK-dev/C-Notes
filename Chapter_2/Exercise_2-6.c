...
int main()
{
    ...
    do {
        if (input_type == 2) {
            input_size = INT_BIN_SIZE - 1;
            printf("\nType the position \"p\" (0...%d): ", input_size);
        } else {
            input_size = INT_HEX_SIZE - 1;
            printf("\nType the position \"p\" (0...%d): ", input_size);
        }
    } while ( ( p = atoi(getline(input)) ) < 0 || p > input_size);

    /* get the `n` number of bits to replace from `x` */
    do {
        printf("\nType the number of bits \"n\" (0...%d): ", p);
    } while ( ( n = atoi(getline(input)) ) < 0 || n > p);
