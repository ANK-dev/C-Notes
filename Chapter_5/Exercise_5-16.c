/*
 * Exercise 5-16 - K&R 2nd ED
 * -- ANK-dev
 *
 * qcksort:  directory order '-d' flag support
 *
...
int readlines(char *lineptr[], int nlines, int fold, int dir_ord);
int getline(char s[], int lim, int fold, int dir_ord);
...
int readlines(char *lineptr[], int maxlines, int fold, int dir_ord)
{
	int len, nlines;
	char line[MAXLEN];
	char *p;
	...
	nlines = 0;
	while ((len = getline(line, MAXLEN, fold, dir_ord)) > 0)
...
int getline(char s[], int lim, int fold, int dir_ord)
{
	...
	for (i = 0; i < lim - 1 && (c = getchar()) != EOF && c != '\n'; i++)
		if (dir_ord) {
			...
			/* avoid skipping indices on the array */
				s[i] = ' ';
		} else if (fold)
			s[i] = tolower(c);
		else
